...
internal class FFB
{
  ...
  public void OnVirtualFFBDataReceived(IntPtr data, object userData)
  {
    if (FFB.StopFFB)
      return;
    this.PacketHandler.ProcessFFBPacket(data, userData, new Action<FFBEventArgs>(this.OnFFBDataReceived), FFB.FFBDevice);
  }
  ...
  private static void LoadSettings()
  {
    try
    {
      Controller controller = InputCollector.Controllers.Where<Controller>((Func<Controller, bool>) (x => x.InstanceGuid == FFB.FFBDevice.Information.InstanceGuid)).Select<Controller, Controller>((Func<Controller, Controller>) (x => x)).First<Controller>();
  ...
  private static bool LoadEffects()
  {
    ...
    {
      ConsoleMsg msg = ConsoleMsg.Msg;
      msg.Message = $"{msg.Message}[ERROR] Could not load Force Feedback effects in memory of  '{FFB.FFBDevice.Information.InstanceName}' {Environment.NewLine}";
      return false;
    }
  }
  ...
  private static void SendConstant(Joystick FFBDevice)
  {
    ...
    int num1 = (int) Math.Round((double) FFBPacketHandler.ConstantForceReport.Magnitude * (double) FFB.ConstantMagnitudeMulti, 0);
    ...
    if (FFBPacketHandler.EffectReport.Duration == ushort.MaxValue || FFBPacketHandler.EffectReport.Duration == (ushort) 0)
      effectParameters.Duration = -1;
    effectParameters.Flags = EffectFlags.ObjectIds | EffectFlags.Cartesian;
    effectParameters.Gain = (int) Math.Round((double) FFBPacketHandler.EffectReport.Gain / (double) byte.MaxValue * 10000.0, 0);
  ...
  private static void SendPeriodic(Joystick FFBDevice)
  {
    ...
    specificParameters.As<PeriodicForce>().Period = (int) Math.Round((double) (FFBPacketHandler.PeriodicReport.Period * 1000U) / (double) FFB.SineFrequencyMulti, 0);
    specificParameters.As<PeriodicForce>().Phase = (int) ((double) FFB.SineWavePhase * 35999.0);
    EffectParameters effectParameters = new EffectParameters()
    {
      Parameters = specificParameters
    };
    FFB.EffectDictionary.Where<KeyValuePair<string, Guid>>((Func<KeyValuePair<string, Guid>, bool>) (x => x.Key == FFBPacketHandler.EffectReport.EffectType.ToString())).Select<KeyValuePair<string, Guid>, Guid>((Func<KeyValuePair<string, Guid>, Guid>) (x => x.Value)).First<Guid>();
    int[] axes = new int[1]{ FFB.ActuatorsObjectTypes[0] };
    int[] directions = new int[1]
    {
      (int) FFBPacketHandler.EffectReport.Direction
    ...
    if (FFBPacketHandler.EffectReport.Duration == ushort.MaxValue || FFBPacketHandler.EffectReport.Duration == (ushort) 0)
      effectParameters.Duration = -1;
    effectParameters.Flags = EffectFlags.ObjectIds | EffectFlags.Cartesian;
    int num1 = (int) Math.Round((double) FFBPacketHandler.EffectReport.Gain / (double) byte.MaxValue * 10000.0 * (double) FFB.SineWaveMagnitude, 0);
    ...
    if (FFBPacketHandler.EffectReport.Gain == (byte) 32 /*0x20*/)
    {
      ...
      specificParameters.As<PeriodicForce>().Period = (int) Math.Round((double) (FFBPacketHandler.PeriodicReport.Period * 1000U) / (double) FFB.GearShiftVibrationFrequency, 0);
    }
    ...
    if (FFBPacketHandler.EffectReport.Gain == (byte) 1)
    {
      ...
      specificParameters.As<PeriodicForce>().Period = (int) Math.Round((double) (FFBPacketHandler.PeriodicReport.Period * 1000U) / (double) FFB.EngineVibrationFrequency, 0);
    }
  ...
  private static void SendCondition(Joystick FFBDevice)
  {
    ...
    specificParameters.As<ConditionSet>().Conditions[0].Offset = (int) FFBPacketHandler.ConditionalReport.CenterPointOffset;
    float num1 = 1f;
    float num2 = 1f;
    ...
    effectGuid = FFB.EffectDictionary.Where<KeyValuePair<string, Guid>>((Func<KeyValuePair<string, Guid>, bool>) (x => x.Key == FFBPacketHandler.EffectReport.EffectType.ToString())).Select<KeyValuePair<string, Guid>, Guid>((Func<KeyValuePair<string, Guid>, Guid>) (x => x.Value)).First<Guid>();
    Effect effect = new Effect(FFBDevice.CreatedEffects.Where<Effect>((Func<Effect, bool>) (x => x.Guid == effectGuid)).Select<Effect, IntPtr>((Func<Effect, IntPtr>) (x => x.NativePointer)).First<IntPtr>());
    int[] axes = new int[1]{ FFB.ActuatorsObjectTypes[0] };
    int[] directions = new int[1]
    {
      (int) FFBPacketHandler.EffectReport.Direction
  ...
  {
    if (FFBPacketHandler.PacketType.ToString() == "PT_NEWEFREP" || FFBPacketHandler.PacketType.ToString() == "PT_SMPLREP" || FFBPacketHandler.PacketType.ToString() == "PT_BLKFRREP" || FFBPacketHandler.PacketType.ToString() == "PT_BLKLDREP" || FFBPacketHandler.PacketType.ToString() == "PT_POOLREP")
      return;
    ...
    else
    {
      if (FFBPacketHandler.EffectReport.EffectType.ToString() == "ET_NONE")
        return;
      if ((FFBPacketHandler.PacketType.ToString() == "PT_EFOPREP" || FFBPacketHandler.PacketType.ToString() == "PT_EFFREP") && FFBPacketHandler.Op.EffectOp.ToString() == "EFF_STOP")
      {
      ...
      if (!(FFBPacketHandler.PacketType.ToString() == "PT_EFOPREP") && !(FFBPacketHandler.PacketType.ToString() == "PT_EFFREP"))
        return;
      ...
      {
        if (!(FFBPacketHandler.EffectReport.EffectType.ToString() == "ET_DMPR") && !(FFBPacketHandler.EffectReport.EffectType.ToString() == "ET_SPRNG"))
          return;
