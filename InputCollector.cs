...
internal class InputCollector
{
    ...
    public DPadData GetDPadData()
    {
        ...
        foreach (Controller controller1 in InputCollector.Controllers)
        {
            ...
            if (controller.DPad != null)
            {
                ...
                if (dpad != null)
                {
                    dpadData.ControllerIndex = InputCollector.GameControllers.FindIndex(
                        (Predicate<Joystick>)(x => x.Information.InstanceGuid == controller.InstanceGuid));
    ...
    public List<ButtonData> GetButtonData(int index)
    {
        ...
        foreach (Controller controller1 in InputCollector.Controllers)
        {
            ...
            if (controller.Buttons != null)
            {
                ...
                if (controller.Buttons.Where<Button>((Func<Button, bool>)(x => x.Id == (Button.ButtonEnum)index))
                    .Select<Button, Button>((Func<Button, Button>)(x => x)).Any<Button>())
                    buttonList = controller.Buttons
                        .Where<Button>((Func<Button, bool>)(x => x.Id == (Button.ButtonEnum)index))
                        .Select<Button, Button>((Func<Button, Button>)(x => x)).ToList<Button>();
                foreach (Button button in buttonList)
                {
                    ButtonData buttonData2 = new ButtonData()
                    {
                        ...
                        ControllerIndex = InputCollector.GameControllers.FindIndex(
                            (Predicate<Joystick>)(x => x.Information.InstanceGuid == controller.InstanceGuid))
                    };
    ...
    public AxisData GetAxisData(string axis)
    {
        ...
        foreach (Controller controller1 in InputCollector.Controllers)
        {
            ...
            {
                Axis axis1 = controller.Axes.Where<Axis>((Func<Axis, bool>)(x => x.Id == axis))
                    .Select<Axis, Axis>((Func<Axis, Axis>)(x => x)).FirstOrDefault<Axis>();
                if (axis1 != null)
                {
                    axisData.AxisIndex = axis1.AxisIndex;
                    axisData.Deadzone = axis1.Deadzone;
                    axisData.Inverted = axis1.Inverted;
                    axisData.ControllerIndex = InputCollector.GameControllers.FindIndex(
                        (Predicate<Joystick>)(x => x.Information.InstanceGuid == controller.InstanceGuid));
                    return axisData;
                }
            }
