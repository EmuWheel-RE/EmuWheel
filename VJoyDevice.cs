...
internal class VJoyDevice
{
    ...
    public static bool InitializeVJoy()
    {
        ...
        if (!VJoyDevice.Joystick.vJoyEnabled())
        {
            ...
            msg.Message =
                $"{msg.Message}[ERROR] vJoy driver not enabled: Failed Getting vJoy attributes.{Environment.NewLine}";
            return false;
        }
        ...
        switch (vjdStatus)
        {
            case VjdStat.VJD_STAT_OWN:
                ...
                msg1.Message =
                    $"{msg1.Message}[ERROR] vJoy Device {(object)VJoyDevice.ID} is already owned by this feeder.{Environment.NewLine}";
                return false;
            case VjdStat.VJD_STAT_FREE:
                ...
                foreach (KeyValuePair<string, bool> keyValuePair in dictionary)
                {
                    if (!keyValuePair.Value)
                    {
                        ...
                        msg2.Message =
                            $"{msg2.Message}[ERROR] vJoy Device {(object)VJoyDevice.ID} is missing axis: {keyValuePair.Key}{Environment.NewLine}";
                        return false;
                    }
                }
                ...
                if (vjdButtonNumber != 128 /*0x80*/)
                {
                    ...
                    msg3.Message =
                        $"{msg3.Message}[ERROR] vJoy Device {(object)VJoyDevice.ID}  has incorrect number of buttons: Expected: {(object)128 /*0x80*/} Reported: {(object)vjdButtonNumber}{Environment.NewLine}";
                    return false;
                }
                ...
                if (vjdDiscPovNumber != 1)
                {
                    ...
                    msg4.Message =
                        $"{msg4.Message}[ERROR] vJoy Device {(object)VJoyDevice.ID}  has incorrect number of 4-way hat switches (Dpads): Expected: {(object)1} Reported: {(object)vjdDiscPovNumber}{Environment.NewLine}";
                    return false;
                }
                ...
                if (vjdContPovNumber > 0)
                {
                    ...
                    msg5.Message =
                        $"{msg5.Message}[ERROR] vJoy Device {(object)VJoyDevice.ID}  has incorrect hat switch type: Expected: 4-way Reported: Continuous{Environment.NewLine}";
                    return false;
                }
                ...
                if (!VJoyDevice.Joystick.DriverMatch(ref DllVer, ref DrvVer))
                {
                    ConsoleMsg.Msg.Message +=
                        "[ERROR] vJoy driver version does not match the version of 'vJoyInterface.dll'";
                    return false;
                }
                ...
                switch (vjdStatus)
                {
                    case VjdStat.VJD_STAT_OWN:
                        ...
                        msg6.Message =
                            $"{msg6.Message}[ERROR] Failed to acquire vJoy device number {(object)VJoyDevice.ID}";
                        return false;
                    case VjdStat.VJD_STAT_FREE:
                ...
                msg7.Message =
                    $"{msg7.Message}[SUCCESS] vJoy device number {(object)VJoyDevice.ID} acquired!{Environment.NewLine}";
                return true;
            case VjdStat.VJD_STAT_BUSY:
                ConsoleMsg msg8 = ConsoleMsg.Msg;
                msg8.Message =
                    $"{msg8.Message}[ERROR] vJoy Device {(object)VJoyDevice.ID} is already owned by another feeder. Cannot continue.{Environment.NewLine}";
                return false;
            case VjdStat.VJD_STAT_MISS:
                ConsoleMsg msg9 = ConsoleMsg.Msg;
                msg9.Message =
                    $"{msg9.Message}[ERROR] vJoy Device {(object)VJoyDevice.ID} is disabled. Cannot continue.{Environment.NewLine}";
                return false;
            default:
                ConsoleMsg msg10 = ConsoleMsg.Msg;
                msg10.Message =
                    $"{msg10.Message}[ERROR] vJoy Device {(object)VJoyDevice.ID} general error. Cannot continue.{Environment.NewLine}";
                return false;
        }
    }
}