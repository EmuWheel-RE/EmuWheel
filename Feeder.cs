...
internal class Feeder
{
  ...
  private ControllerMapping GetOutputMapping()
  {
    ...
    outputMapping.Buttons = source.OrderBy<ButtonData, int>((Func<ButtonData, int>) (x => x.ButtonBind)).ToArray<ButtonData>();
    ...
    while (Feeder.RunFeeder)
    {
      ...
      foreach (Joystick controller in controllers)
      {
        ...
        catch
        {
          ...
          msg.Message = $"{msg.Message}[ERROR] Could not poll device '{controller.Information.InstanceName}'. Forza EmuWheel has stopped...{Environment.NewLine}";
  ...
  public void FeedData(List<JoystickState> data, ControllerMapping mapping)
  {
    if (mapping.Steering != null)
    {
      ...
      if (sliderValue == -1)
      {
        sliderValue = (int) data[mapping.Steering.ControllerIndex].GetType().GetProperty(axisIndex.ToString()).GetValue((object) data[mapping.Steering.ControllerIndex]);
    ...
    if (mapping.Combined != null)
    {
      ...
      int sliderValue = this.GetSliderValue(axisIndex, data, controllerIndex);
      if (sliderValue == -1)
        sliderValue = (int) data[mapping.Combined.ControllerIndex].GetType().GetProperty(axisIndex.ToString()).GetValue((object) data[mapping.Combined.ControllerIndex]);
    ...
    if (mapping.Throttle != null)
    {
      ...
      int sliderValue = this.GetSliderValue(axisIndex, data, controllerIndex);
      if (sliderValue == -1)
        sliderValue = (int) data[mapping.Throttle.ControllerIndex].GetType().GetProperty(axisIndex.ToString()).GetValue((object) data[mapping.Throttle.ControllerIndex]);
    ...
    if (mapping.Brake != null)
    {
      ...
      int sliderValue = this.GetSliderValue(axisIndex, data, controllerIndex);
      if (sliderValue == -1)
        sliderValue = (int) data[mapping.Brake.ControllerIndex].GetType().GetProperty(axisIndex.ToString()).GetValue((object) data[mapping.Brake.ControllerIndex]);
    ...
    if (mapping.Clutch != null)
    {
      ...
      int sliderValue = this.GetSliderValue(axisIndex, data, controllerIndex);
      if (sliderValue == -1)
        sliderValue = (int) data[mapping.Clutch.ControllerIndex].GetType().GetProperty(axisIndex.ToString()).GetValue((object) data[mapping.Clutch.ControllerIndex]);
    ...
    if (mapping.Handbrake != null)
    {
      ...
      int sliderValue = this.GetSliderValue(axisIndex, data, controllerIndex);
      if (sliderValue == -1)
        sliderValue = (int) data[mapping.Handbrake.ControllerIndex].GetType().GetProperty(axisIndex.ToString()).GetValue((object) data[mapping.Handbrake.ControllerIndex]);
  ...
  private int GetSliderValue(Axis.AxisEnum axis, List<JoystickState> data, int index)
  {
    ...
    if (axis.ToString().IndexOf("Sliders") == -1)
      return sliderValue;
    int[] numArray = (int[]) data[index].GetType().GetProperty(axis.ToString().Remove(7)).GetValue((object) data[index]);
  ...
  private bool[] GetButtonArray(List<JoystickState> data, ControllerMapping mapping)
  {
    ...
    for (int index = 0; index < ((IEnumerable<ButtonData>) mapping.Buttons).Count<ButtonData>(); ++index)
    {
      if (!buttonArray[mapping.Buttons[index].ButtonBind])
        buttonArray[mapping.Buttons[index].ButtonBind] = data[mapping.Buttons[index].ControllerIndex].Buttons[mapping.Buttons[index].ButtonIndex];
    }
    return buttonArray;
  }
  ...
  private void ResetVJoyState()
  {
    ...
    uint uint32_2 = Convert.ToUInt32(Utils.PackBoolsInByteArray(Enumerable.Repeat<bool>(true, 4).ToArray<bool>())[0]);
