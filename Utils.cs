    ...
    public static int CombinedAxisSplit(int value, AxisData mapping, int inverted)
    {
        if (value > (int)short.MaxValue + (int)((double)mapping.Deadzone * (double)ushort.MaxValue / 2.0))
        {
            value = (int)Math.Round(
                (((double)value - (double)mapping.Deadzone * (double)ushort.MaxValue) /
                    (1.0 - (double)mapping.Deadzone) - (double)short.MaxValue) * 2.0);
            if (inverted == 0)
    ...
    public static int DeadzoneAndInvert(int value, AxisData mapping, bool steering)
    {
        ...
        if (steering)
        {
            if (value < (int)short.MaxValue &&
                Math.Abs(value - (int)short.MaxValue) < (int)((double)mapping.Deadzone * 32768.0))
                value = (int)short.MaxValue;
            else if (value > (int)short.MaxValue &&
                     value - (int)short.MaxValue < (int)((double)mapping.Deadzone * 32768.0))
                value = (int)short.MaxValue;
            else if (value < (int)short.MaxValue - (int)((double)mapping.Deadzone * (double)ushort.MaxValue / 2.0))
            {
                value = Math.Abs(value - (int)ushort.MaxValue);
                value = (int)Math.Round(
                    (double)(value - (int)((double)mapping.Deadzone * (double)ushort.MaxValue)) /
                    (double)((int)ushort.MaxValue - (int)((double)mapping.Deadzone * (double)ushort.MaxValue)) *
                    (double)ushort.MaxValue, 0);
                value = Math.Abs(value - (int)ushort.MaxValue);
            }
            else if (value > (int)short.MaxValue + (int)((double)mapping.Deadzone * (double)ushort.MaxValue / 2.0))
                value = (int)Math.Round(
        ...
        else if (mapping.Inverted == 0 && value > Math.Abs((int)((double)mapping.Deadzone * (double)ushort.MaxValue)) &&
                 (double)mapping.Deadzone != 0.0)
            value = (int)ushort.MaxValue;
        else if (mapping.Inverted == 1 && value < (int)((double)mapping.Deadzone * (double)ushort.MaxValue) &&
                 (double)mapping.Deadzone != 0.0)
            value = 0;
